...
void solve(){
    ...
    vi aa(n,mxn),bb(n,mxn);
    if(ans[0]!=0) aa[0]=ans[0];
    if(ans[n-1]!=0) bb[n-1]=n-1+ans[n-1];
    for(int i=1;i<n;i++){
		if(ans[i]==0){
			aa[i]=aa[i-1];
		}
		else{
            aa[i]=min(aa[i-1],ans[i]-i);
        }
	}
	for(int i=n-2;i>=0;i--)
	{
		if(ans[i]==0){
			...
			bb[i]=min(bb[i+1],ans[i]+i);
		}
	}
    for(int i=0;i<n;i++){
		ans[i] = min(bb[i]-i,aa[i]+i);
	}
    for(auto i:ans){
        cout<<i<<' ';
    }
