...
void print(vector<vector<ll>> &a, vector<vector<ll>> &b, ll r, ll c){
  
  if(r==-1) return;
  if(c==0){ print(a,b,r-1,a[0].size()-1); if(r!=0) cout << '\n'; }
  else print(a,b,r,c-1);
  cout << a[r][c]+b[r][c] << ' ';
...
void solve(){
  ...
  fo(i,0,r){fo(j,0,c){cin >> a[i][j];}}
 
  fo(i,0,r){fo(j,0,c){cin >> b[i][j];}}  
