...
void solve(){
    ...
    for(int i=2;i<=n;i++){
        ...
        for(int j=2;j<=n && j!=i;j++){
            if((double)(a[j]-a[1])/(double)(j-1) == slope) pts.erase(j);
        }
        vector<int> check(all(pts));
        int m=check.size();
        debug(check);
        set<double> siz;
        for(int i=1;i<m;i++){
            siz.insert((double)(a[check[i]]-a[check[i-1]])/(double)(check[i]-check[i-1]));
        }
        debug(siz);
        // 3,3 2,4 1,1
        if(siz.size()==1 && m>1 && *siz.begin()==slope) {cout<<"Yes";return;}
    }
    set<double> ss;
    
    for(int i=3;i<=n;i++){
            ss.insert((double)(a[i]-a[i-1]));
    }
    debug(ss);
