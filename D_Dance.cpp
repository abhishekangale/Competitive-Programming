...
const int mod = 1e9 + 7;
int ans = 0;
void generatePairings(vi &items, int itemcount, int start, vvi &a)
{
    if(itemcount & 1)
        return;
    if(start == itemcount)
    {
        ...
        {
            int mn = min(items[i], items[i+1]);
            int mx = max(items[i], items[i+1]);
            xo ^= a[mn][mx];
        }
        ans = max(ans, xo);
        return;
    ...
    for(j = start+1; j<itemcount; j++)
    {
        ...
        items[j] = temp;
        generatePairings(items, itemcount, start+2, a);
        temp = items[start+1];
        items[start+1] = items[j];
        items[j] = temp;
    }
