...
void solve(){
    ...
    vi spell;
    for(int i = 0; i<n; i++){
        spell.pb(store[i].F - store[i].S + 1);
    }
    int mn = spell[n-1];
    for(int i = n-1; i>=0; i--){
        mn = min(mn, spell[i]);
    ...
    // for(int i = 0; i<n; i++){
    //     if(i == 0){
    //         an[i] = (store[i].F - spell[i] + 1);
    //         continue;
    //     }
    //     else {
    //         if(spell[i] == spell[i-1]){
    //             an[i] = an[i-1] + store[i].F - store[i-1].F;
    //         }
    //         else {
    //             an[i] = (store[i].F - spell[i] + 1);
    //         }
    ...
    for(int i = 0; i<n; i++){
        if(spell[i]==-1)continue;
        int add = store[i].F - spell[i] + 1;
