...
const int mod = 1e9 + 7;
int get_cnt(string x){
	int ans=0;
	for(int i=0;i<x.size();i++) if(x.substr(i,4)=="haha") ans++;
	return ans; 
}
string pre(string x){
     return x.substr(0,3);
...
string suf(string x){ 
    if(x.size()<3) return x.substr(0,3);
    else return x.substr(x.size()-3);
} 
void solve(){
    int n;
    cin>>n;
    ...
    for(int i=0;i<n;i++){
        ...
        if(op[0]==':'){
            ...
            string x,y,z;
            cin>>x>>y>>z;
            cnt[i]=get_cnt(r[a[x]]+l[a[z]])+cnt[a[x]]+cnt[a[z]];
            l[i] = pre(l[a[x]]+l[a[z]]);
            r[i] = suf(r[a[x]]+r[a[z]]);
        }
        a[var]=i;
