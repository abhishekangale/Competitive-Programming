...
const int mod = 1e9 + 7;
int ans = 0;
void generatePairings(vi &items, int itemcount, int start, int target)
{
    if(itemcount & 1)
        return;
    if(start == itemcount)
    ...
    for(j = start+1; j<itemcount; j++)
    {
        ...
        items[j] = temp;
        generatePairings(items, itemcount, start+2, target);
        temp = items[start+1];
        items[start+1] = items[j];
        items[j] = temp;
    }
...
void solve(){
    ...
    vi xd;
    for(int i = 0; i < n; i++)xd.pb(i);
    generatePairings(xd, n, 0 , k);
}
signed main(){
